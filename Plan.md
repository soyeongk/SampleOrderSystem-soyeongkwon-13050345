...
**추가 수정 (같은 슬라이스 내 반복)**: `pad()`에 최소 여백을 강제로 넣었더니, 폭을 채우거나
초과하는 셀이 있는 행만 다음 컬럼이 다른 행보다 오른쪽으로 밀리는 문제가 발생함 (고정 폭이
실제 내용보다 좁아서 생기는 문제). **컬럼 폭을 하드코딩하지 않고, 그 페이지의 헤더/셀 중
최대 표시 폭으로 동적으로 계산**하도록 재설계 — 이렇게 하면 어떤 셀도 배정된 폭을 넘지 않아
행 간 정렬이 항상 일치한다.

- `views/table.py`에 `compute_column_widths(headers, rows)`(컬럼별 최대 표시 폭 계산),
  `render_row(cells, widths, gap=2)`(그 폭 + 여백으로 셀을 이어붙임) 추가. `pad()`는 강제
